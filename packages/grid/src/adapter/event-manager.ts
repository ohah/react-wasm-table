...
export interface GridEventHandlers {
	...
	/** Fires on hover over data cells (not during drag). null when not over a cell or over a resize handle. */
...
export class EventManager {
	...
	hitTestAtLastPos(canvasWidth?: number): CellCoord | null {
		...
		return (
			findCell(x, y, this.rowLayouts) ?? findNearestCell(x, y, this.rowLayouts)
		);
	}
	...
	attach(
		...
		const buildHitTest = (
			x: number,
			y: number,
		): import("../types").HitTestResult => {
			const headerHit = findCell(x, y, this.headerLayouts);
			if (headerHit) {
				const resizeCol = this.findResizeHandle(x, y);
				if (resizeCol !== -1)
					return { type: "resize-handle", colIndex: resizeCol };
		...
		canvas.addEventListener(
			...
			(e: MouseEvent) => {
				...
				if (handlers.onCanvasEvent) {
					...
					if (handlers.onCanvasEvent("click", e, hitTest, coords) === false)
						return;
				}
		...
		canvas.addEventListener(
			...
			(e: MouseEvent) => {
				...
				if (handlers.onCanvasEvent) {
					const hitTest = buildHitTest(x, y);
					if (handlers.onCanvasEvent("dblclick", e, hitTest, coords) === false)
						return;
				}
		...
		canvas.addEventListener(
			...
			(e: MouseEvent) => {
				...
				if (handlers.onCanvasEvent) {
					const hitTest = buildHitTest(x, y);
					if (handlers.onCanvasEvent("mousedown", e, hitTest, coords) === false)
						return;
				}
		...
		window.addEventListener(
			...
			(e: MouseEvent) => {
				...
				if (handlers.onCanvasEvent) {
					const hitTest = buildHitTest(x, y);
					if (handlers.onCanvasEvent("mousemove", e, hitTest, coords) === false)
						return;
				}
		...
		// mousemove on canvas for resize handle hover cursor + editable cell cursor
		canvas.addEventListener(
			"mousemove",
			(e: MouseEvent) => {
				if (e.buttons & 1) return; // skip during drag — handled by window mousemove
				const { contentX: x, contentY: y } = toContentCoords(
					e.clientX,
					e.clientY,
				);
				const resizeCol = this.findResizeHandle(x, y);
		...
		const syntheticMouse = (
			...
			clientY: number,
			type = "click",
		): MouseEvent => new MouseEvent(type, { clientX, clientY, bubbles: true });
				...
				this.touchState = {
					...
					longPressTimer: setTimeout(() => {
						...
						if (Math.sqrt(dx * dx + dy * dy) < TAP_THRESHOLD) {
							...
							const lpc = touchToCoords(
								this.touchState.lastX,
								this.touchState.lastY,
							);
							const hdrHit = findCell(
								lpc.contentX,
								lpc.contentY,
								this.headerLayouts,
							);
							const hit =
								hdrHit ?? findCell(lpc.contentX, lpc.contentY, this.rowLayouts);
							if (hit) {
		...
		canvas.addEventListener(
			...
			(e: TouchEvent) => {
				...
				if (dist < TAP_THRESHOLD && elapsed < TAP_DURATION) {
					...
					if (
						timeSinceLastTap < DOUBLE_TAP_INTERVAL &&
						...
						if (headerHit) {
							...
							const nativeDown = syntheticMouse(
								ts.startX,
								ts.startY,
								"mousedown",
							);
							handlers.onCellMouseDown?.(
								headerHit,
								false,
								nativeDown,
								tapCoords,
