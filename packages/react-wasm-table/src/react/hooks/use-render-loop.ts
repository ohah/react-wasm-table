      ...
      if (dirtyRef.current) {
        ...
        if (ctx) {
          const getInstruction = (cellIdx: number) => {
            ...
            const actualRow =
              effectiveViewIndices[readCellRow(layoutBuf, cellIdx) - headerRowCount] ?? 0;
          ...
          for (let i = headerCount; i < cellCount; i++) {
            ...
            if (
              inst &&
              (inst.type === "input" || inst.type === "select" || inst.type === "datepicker")
            ) {
          ...
          if (parsedBorderStyles || hasColumnBorder) {
            ...
            for (let i = headerCount; i < cellCount; i++) {
              ...
              const actualRow =
                effectiveViewIndices[readCellRow(layoutBuf, i) - headerRowCount] ?? 0;
