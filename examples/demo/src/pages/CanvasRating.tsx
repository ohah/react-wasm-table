...
import { Grid, createColumnHelper, Rating, type GridCellEvent } from "@ohah/react-wasm-table";
  ...
  const columns = useMemo(
    () => [
      ...
      helper.accessor("score", {
        ...
        size: 200,
        cell: (info) => {
          const val = Math.round((info.getValue() / 100) * max);
          return (
            <Rating
              value={val}
              max={max}
              color={color}
              onClick={(e) => addLog("onClick", `${val}/${max}`, e)}
              onMouseEnter={(e) => addLog("onMouseEnter", `${val}/${max}`, e)}
              onMouseLeave={(e) => addLog("onMouseLeave", `${val}/${max}`, e)}
            />
          );
        },
      }),
