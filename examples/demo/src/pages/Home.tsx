...
const DEPARTMENTS = ["Engineering", "Product", "Design", "Marketing", "Sales", "HR", "Finance", "Ops"];
const STATUSES = ["Active", "On Leave", "Inactive", "Probation"];
const STATUS_COLORS: Record<string, string> = {
  Active: "#4caf50", "On Leave": "#ff9800", Inactive: "#9e9e9e", Probation: "#2196f3",
};
...
const FIRST = ["Alice", "Bob", "Charlie", "Diana", "Eve", "Frank", "Grace", "Hank", "Ivy", "Jack", "Karen", "Leo", "Mia", "Noah", "Olivia", "Paul", "Quinn", "Ruby", "Sam", "Tina"];
const LAST = ["Kim", "Lee", "Park", "Choi", "Smith", "Johnson", "Brown", "Garcia", "Miller", "Davis", "Tanaka", "Sato"];
...
export function Home() {
  ...
  const columns = useMemo(() => [
    helper.accessor("id", {
      header: "#", size: 55, align: "right", enableSorting: true, padding: [0, 8],
    }),

    // Text — varied colors/weights per row
    helper.accessor("name", {
      header: "Name (Text)", size: 160, enableSorting: true, padding: [0, 8],
      cell: (info) => {
    ...
    helper.accessor("status", {
      header: "Status (Badge)", size: 120, enableSorting: true, padding: [0, 6],
      cell: (info) => {
    ...
    helper.accessor("tags", {
      header: "Tag", size: 110, padding: [0, 8],
      cell: (info) => {
        const v = info.getValue();
        const s = TAG_STYLES[v] ?? { color: "#666", borderColor: "#999" };
        return <Tag value={v} color={s.color} borderColor={s.borderColor} borderRadius={4} fontSize={11} />;
      },
    }),

    // Chip — unique color per department
    helper.accessor("department", {
      header: "Dept (Chip)", size: 130, enableSorting: true, padding: [0, 6],
      cell: (info) => {
        const v = info.getValue();
    ...
    helper.accessor("salary", {
      header: "Salary (Flex)", size: 170, enableSorting: true, align: "right", padding: [0, 10],
      cell: (info) => {
        const v = info.getValue();
        const high = v >= 150000;
        const mid = v >= 80000;
        return (
          <Flex flexDirection="row" gap={4} alignItems="center" justifyContent="flex-end">
            <Text value="$" color={high ? "#2e7d32" : (isDark ? "#999" : "#888")} fontSize={11} fontWeight={high ? "bold" : "normal"} />
            <Text
              value={v.toLocaleString()} fontWeight="bold"
              fontSize={high ? 14 : 13}
    ...
    helper.accessor("score", {
      header: "Score (Progress)", size: 190, enableSorting: true, padding: [0, 8],
      cell: (info) => {
        const v = info.getValue();
    ...
    helper.accessor("trend", {
      header: "Trend (Sparkline)", size: 140,
      cell: (info) => {
        const idx = info.row.index;
    ...
    helper.accessor("rating", {
      header: "Rating", size: 110, enableSorting: true, padding: [0, 6],
      cell: (info) => {
        const v = info.getValue();
        const c = v >= 4 ? "#f59e0b" : v >= 2 ? "#fb923c" : "#94a3b8";
        return (
          <Rating
            value={v} max={5} color={c} size={15}
    ...
    helper.accessor("color", {
      header: "Color", size: 55, align: "center",
      cell: (info) => {
        const idx = info.row.index;
        return (
          <Color
    ...
    helper.accessor("isActive", {
      header: "Switch", size: 65, align: "center",
      cell: (info) => {
        const idx = info.row.index;
    ...
    helper.accessor("approved", {
      header: "OK", size: 55, align: "center",
      cell: (info) => {
        const v = info.getValue();
        return (
          <Badge
    ...
    helper.accessor("website", {
      header: "Link", size: 85, padding: [0, 8],
      cell: (info) => {
        const idx = info.row.index;
  ...
  return (
    <>
      ...
      <p style={{ color: "var(--demo-muted, #666)", fontSize: 13 }}>
        Try: click <strong>Switch</strong> to toggle &middot; drag <strong>Progress bar</strong> to change score &middot; click <strong>Rating</strong> stars to cycle &middot; click <strong>OK</strong> badge to toggle &middot; click headers to sort
      </p>
